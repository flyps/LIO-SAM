...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions &options) : ParamServer("lio_sam_mapOptimization", options) {
        ...
        pubCloudRegisteredRaw = create_publisher<sensor_msgs::msg::PointCloud2>("lio_sam/mapping/cloud_registered_raw",
                                                                                1);
    ...
    void updateInitialGuess() {
        ...
        if (cloudInfo.odom_available == true) {
            ...
            if (lastImuPreTransAvailable == false) {
                ...
                lastImuTransformation = pcl::getTransformation(0, 0, 0, cloudInfo.imu_roll_init,
        ...
        if (cloudInfo.imu_available == true) {
            Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo.imu_roll_init,
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void publishOdometry() {
        ...
        // Publish TF in the baselinkFrame (the same as imuPreintegration odom)
        quat_tf.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
        tf2::Transform t_odom_to_lidar = tf2::Transform(quat_tf,
        ...
        if (lastIncreOdomPubFlag == false) {
            ...
            if (cloudInfo.imu_available == true) {
                if (std::abs(cloudInfo.imu_pitch_init) < 1.4) {
                    ...
                    imuQuaternion.setRPY(cloudInfo.imu_roll_init, 0, 0);
                    tf2::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid,
                                                                                               yawMid);
                    roll = rollMid;
